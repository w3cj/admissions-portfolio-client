<template lang="html">
  <v-expansion-panel expand class="applicant-list mt-4 mb-4">
    <v-expansion-panel-content v-for="applicant in sortedApplicants" :key="applicant._id" v-model="applicant.view">
      ...
      <v-card>
        ...
        <div v-for="portfolio in portfolios[applicant._id]">
          ...
          <v-btn v-if="!hideArchiveButton" error @click.native="archiveApplicantDialog($event, applicant)">Archive Applicant<v-icon right>delete</v-icon></v-btn>
        </div>
      </v-card>
    </v-expansion-panel-content>
  </v-expansion-panel>
